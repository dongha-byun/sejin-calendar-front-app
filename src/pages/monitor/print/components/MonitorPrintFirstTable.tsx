import type { PrintPutinDetailDto, PrintPutinSummaryDto } from "../../../../types/monitor/MonitorPrintStockResponse";

interface Props {
    isDetail: boolean;
    detail: PrintPutinDetailDto[];
    summary: PrintPutinSummaryDto[];
}

function DetailTable({ data }: { data: PrintPutinDetailDto[] }) {
    return (
...
function SummaryTable({ data }: { data: PrintPutinSummaryDto[] }) {
    return (
        <table className="w-full table-auto border text-sm">
            ...
            <tbody>
            {data.map((s, idx) => (
                <tr key={`${s.weight}-${s.properties}-${s.standard}`} className={idx % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                    <td className="border px-2 py-1 text-right">{s.weight}</td>
                    <td className="border px-2 py-1">{s.properties}</td>
                    <td className="border px-2 py-1">{s.standard}</td>
...
export default function MonitorPrintFirstTable({ isDetail, detail, summary }: Props) {
    return (
