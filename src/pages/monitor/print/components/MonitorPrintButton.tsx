import type { PrintCompanyStatistics } from "../../../../types/monitor/MonitorPrintStockResponse";

interface Props {
    statistics: PrintCompanyStatistics | undefined;
    onInit: () => void;
}

export default function MonitorPrintButton({ statistics, onInit }: Props) {
    ...
    return (
        <div className="mt-6 flex flex-col items-center justify-center text-sm gap-3">
            ...
            <div>
                ...
                인쇄소 재고: <span className={color(statistics?.printCompanyStocks)}>{fmt(statistics?.printCompanyStocks)}</span>
            </div>
